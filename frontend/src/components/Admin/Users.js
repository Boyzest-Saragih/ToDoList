        ...
        <table className="w-[1000px] max-sm:w-20 border border-gray-700 text-left rounded-lg">
          <thead className="bg-gray-800">
            <tr className="text-white">
              ...
              <th className="flex justify-center p-3 border border-gray-600">Profile Picture</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user, index) => (
              <tr key={user._id} className="hover:bg-gray-800 transition-all">
                <td className="px-3 py-4 border border-gray-600">{index+1}</td>
                <td className="px-3 py-4 border border-gray-600">{user.name}</td>
                <td className="px-3 py-4 border border-gray-600">{user.email}</td>
                <td className="px-3 py-4 border border-gray-600">
                  {user.image ? (
                    ...
                    <span className="flex justify-center text-gray-400">No Image</span>
                  )}
                </td>
              </tr>
            ))}
